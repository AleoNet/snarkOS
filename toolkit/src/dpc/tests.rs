...
use crate::{
    ...
    dpc::{offline_transaction_execution, Record},
};
...
pub fn execute_context_test() {
    ...
    let execute_context =
        offline_transaction_execution(vec![private_key], vec![record], vec![address], vec![10000], 1, rng);
    assert!(execute_context.is_ok());
}
