    ...
    pub async fn start(self) -> anyhow::Result<()> {
        ...
        if self.is_miner {
            let miner_address = match self.miner_address {
                ...
                None => {
                    Account::<Testnet2>::new(&mut thread_rng()).address()
                }
            };

            let _node =
                Node::<Testnet2, Miner>::new(node_port, rpc_port, (node_port as u16 - 4130) as u8, Some(miner_address)).await?;
        } else {
