...
impl NetworkError {
    ...
    pub fn is_fatal(&self) -> bool {
        match self {
            Self::Io(err) => [ErrorKind::BrokenPipe, ErrorKind::ConnectionReset, ErrorKind::UnexpectedEof].contains(&err.kind()),
            // other critical errors
            // Self::CapnProto(_) |
            Self::MessageTooBig(..) | Self::ZeroLengthMessage | Self::Noise(_) => true,
            _ => false,
        }
    }
