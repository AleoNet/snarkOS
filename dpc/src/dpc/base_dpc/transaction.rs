...
pub struct DPCTransaction<C: BaseDPCComponents> {
    ...
    pub new_records_ciphertext_and_fq_high_selectors: Vec<(Vec<bool>, Vec<bool>)>,
...
impl<C: BaseDPCComponents> DPCTransaction<C> {
    pub fn new(
        ...
        new_records_ciphertext_and_fq_high_selectors: Vec<(Vec<bool>, Vec<bool>)>,
    ) -> Self {
        Self {
            old_serial_numbers,
            new_commitments,
...
impl<C: BaseDPCComponents> ToBytes for DPCTransaction<C> {
    ...
    fn write<W: Write>(&self, mut writer: W) -> IoResult<()> {
        ...
        for (ciphertext, (ciphertext_selector_bits, fq_high_selector_bits)) in self
            .record_ciphertexts
            .iter()
            .zip_eq(&self.new_records_ciphertext_and_fq_high_selectors)
        {
            ...
            let selector_bytes = bits_to_bytes(&[&ciphertext_selector_bits[..], &fq_high_selector_bits[..]].concat());
...
impl<C: BaseDPCComponents> FromBytes for DPCTransaction<C> {
    ...
    fn read<R: Read>(mut reader: R) -> IoResult<Self> {
        ...
        for _ in 0..num_ciphertexts {
            ...
            let fq_high_selector_bits = &selector_bits[num_ciphertext_elements..num_ciphertext_elements * 2];
            ...
            new_records_ciphertext_and_fq_high_selectors
                .push((ciphertext_selector_bits.to_vec(), fq_high_selector_bits.to_vec()));
        }
