...
fn test_execute_base_dpc_constraints() {
    ...
    println!("inner_snark_id: {:?}", inner_snark_id);

    println!("inner_snark_vk len: {:?}", to_bytes![inner_snark_vk].unwrap().len());

    println!(
        "program snark vk len: {:?}\n",
        old_proof_and_vk[0].verification_key.len()
    );
