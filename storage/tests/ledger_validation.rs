...
fn test_ledger_validation() {
    ...
    {
        let ledger = LedgerState::open_writer::<RocksDB, _>(&temp_dir, 1).expect("Failed to initialize ledger");
        for block in &blocks {
    ...
    for _ in 0..NUM_CHECKS {
        ...
        let _ledger: LedgerState<Testnet2> =
            LedgerState::open_writer::<RocksDB, &Path>(&temp_dir, increment).expect("Failed to initialize ledger");
    }
}
