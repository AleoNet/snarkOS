...
//         for serial in transaction.serial_numbers.iter() {
//             self.inner().store(
...
//             transaction_serial_numbers.extend_from_slice(&transaction.serial_numbers[..]);
...
//
//         let mut child_hashes = self.get_block_children(&block.previous_block_hash())?;
//
//         if !child_hashes.contains(&hash) {
//             child_hashes.push(hash);
//
//             self.update_child_block_hashes(&block.previous_block_hash(), &child_hashes[..])?;
//         }
//
//         Ok(())
//     }
