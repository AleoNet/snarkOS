...
NODE_ID=0
while [ -n "$(grep "snarkos-n${NODE_ID}" ~/.ssh/config)" ]; do
    NODE_ID=$((NODE_ID + 1))
done
...
run_in_ssh() {
  ...
  ssh -o StrictHostKeyChecking=no snarkos-n$NODE_ID << EOF
    # Commands to run
    ...
    rm -rf ~/.aleo/storage/ledger-3

    # Move the new ledger into place
    cp -R ledger-3 ~/.aleo/storage/ledger-3
...
EOF
  ...
  if [ $? -eq 0 ]; then
    echo "Script ran successfully on snarkos-n$NODE_ID."
  else
    echo "Failed to run the script on snarkos-n$NODE_ID."
  fi
}
