...
/// A resource that can be managed.
...
impl Clone for Resources {
    fn clone(&self) -> Self {
        Self { sender: self.sender.clone(), index: self.index.clone() }
    }
}
...
impl Resources {
    ...
    pub fn new() -> Self {
                ...
                match request {
                    ResourceRequest::Register(resource, id) => {
                        if resources.insert(id, resource).is_some() {
                            error!("A resource with ID {id} already exists!");
                        }
                    }
        ...
        Self { sender, index: Default::default() }
    }
