...
impl<N: Network, C: ConsensusStorage<N>> Ledger<N, C> {
    ...
    pub fn add_next_block(&self, block: &Block<N>) -> Result<()> {
        ...
        // Acquire the write lock on the current epoch challenge.
        let mut current_epoch_challenge = self.current_epoch_challenge.write();

        // Update the VM.
        self.vm.add_next_block(block)?;
        // Update the epoch challenge.
        let epoch_challenge = self.get_epoch_challenge(block.height()).ok();

        // Update the current block.
        *current_block = block.clone();
        // Update the current epoch challenge.
        current_epoch_challenge.clone_from(&epoch_challenge);
