...
pub trait Inbound<N: Network>: Reading + Outbound<N> {
    ...
    fn peer_request(&self, peer_ip: SocketAddr) -> bool {
        ...
        let peers = match self.router().is_dev() {
            ...
            true => peers.into_iter().filter(|ip| *ip != peer_ip && !is_bogon_ip(ip.ip())).take(u8::MAX as usize).collect(),
            // In production mode, ensure the peer IPs are valid.
            false => peers.into_iter().filter(|ip| *ip != peer_ip && self.router().is_valid_peer_ip(ip)).take(u8::MAX as usize).collect(),
        };
