...
impl Parameters for OuterSNARKPKParameters {
    ...
    fn load_bytes() -> Result<Vec<u8>, ParametersError> {
        ...
        let output = catch_unwind(|| {
            lazy_static_include_bytes!(OUTER_SNARK_PK, "src/outer_snark_pk/outer_snark_pk.params");
            *OUTER_SNARK_PK
        });
        ...
        match output {
            ...
            _ => {
                ...
                if relative_path.exists() {
                    ...
                    // Downloads the missing parameters and attempts to store it in the local directory for use.
                    eprintln!(
