...
impl SyncBase {
    ...
    pub async fn find_sync_nodes(&self) -> Result<Vec<Peer>> {
        ...
        for mut node in self.node.peer_book.connected_peers_snapshot().await {
            ...
            if !judge_bad && node.quality.sync_state.block_height as usize > our_block_height + 1 {
                interesting_peers.push(node);
            }
        }
        interesting_peers.sort_by(|x, y| y.quality.sync_state.block_height.cmp(&x.quality.sync_state.block_height));

        // trim nodes close to us if any are > 10 blocks ahead
        if let Some(i) = interesting_peers
            .iter()
            .position(|x| x.quality.sync_state.block_height as usize <= our_block_height + 10)
        {
