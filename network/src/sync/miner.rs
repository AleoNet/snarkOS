...
use snarkvm_dpc::{testnet1::parameters::*, Address, Storage};
use tokio::task;
use tracing::*;
...
pub struct MinerInstance<S: Storage + core::marker::Sync + Send + 'static> {
    miner_address: Address<Testnet1Parameters>,
    node: Node<S>,
}

impl<S: Storage + Send + Sync + 'static> MinerInstance<S> {
    /// Creates a new MinerInstance for spawning miners.
    pub fn new(miner_address: Address<Testnet1Parameters>, node: Node<S>) -> Self {
